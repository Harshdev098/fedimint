...
fn fixtures()->Fixtures{
    Fixtures::new_primary(DummyClientInit,DummyInit)
        .with_module(EscrowClientInit,EscrowInit)
}
...
async fn fund_client(
    ...
    dummy
        .mock_receive(amount, AmountUnit::BITCOIN)
        .await?;
...
async fn test_buyer_resolution() -> Result<(),anyhow::Error> {
    ...
    let (operation_id,escrow_id)=buyer_escrow.create_escrow(
    ...
    let resolve_op = seller_escrow
        .resolve_escrow(escrow_id, buyer_sig)
        .await?;
...
async fn test_arbiter_resolution() -> Result<(),anyhow::Error> {
    ...
    fedimint_core::task::sleep_in_test(
        "waiting for the escrow timeout",
...
async fn test_arbiter_should_not_act_before_timeout() -> anyhow::Result<(),anyhow::Error> {
    ...
    let (_operation_id, escrow_id) = escrow.create_escrow(
        seller_keypair.public_key(),
        arbiter_keypair.public_key(),
        Amount::from_sats(10),
