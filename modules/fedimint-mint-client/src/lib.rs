...
impl MintClientModule {
    ...
    ) -> anyhow::Result<impl futures::Stream<Item = anyhow::Result<serde_json::Value>> + '_> {
            ...
            while let Some(state) = stream.next().await {
                match state {
                    SpendExactState::Success(notes) => {
                        let individual_tokens: Vec<String> = notes
                            .into_iter_items()
                            .map(|(amount, note)| {
                                let mut single = TieredMulti::default();
                                single.push(amount, note);
                                match &invite_code {
                                    Some(ic) => OOBNotes::new_with_invite(single, ic).to_string(),
                                    None => OOBNotes::new(federation_id_prefix, single).to_string(),
                                }
                            })
                            .collect();

                        yield serde_json::to_value(individual_tokens)?;
                        return;
                    }
