        ...
        match self.state {
            EscrowInputSMState::Pending => {
                ...
                vec![StateTransition::new(
                    ...
                    |dbtx, result, old_state: EscrowInputStateMachine| Box::pin(async move {
                        let new_state= match result {
                            Ok(())=>{
                                match &old_state.common.resolution {
                        ...
                        let escrow_status= match &new_state {
                            ...
                            EscrowInputSMState::Failed { reason } => {
                                EscrowClientStatus::Failed { reason: reason.clone() }
                            }
                            _ => unreachable!(),
                        };
                        
                        dbtx.module_tx().insert_entry(
